...
from core.models import VectorizedItem, Gender, Embedding
...
async def test_item_create(skip_not_integration, qdrant_client, random_embedding):
    try:
        ...
        item = VectorizedItem(item_id, True, True, Gender.FEMALE, random_embedding)
        ...
        assert ret.embedding_ready == True
...
@pytest.mark.asyncio
async def test_item_partial_create(skip_not_integration, qdrant_client):
    try:
        migrator = QdrantMigrator(qdrant_client)
        await migrator.create_collections_if_needed()
        item_repo = QdrantItemRepo(qdrant_client)
        ...
        item_id = uuid.uuid4()
        embedding = Embedding([0.0] * EMBEDDING_LENGTH)

        item = VectorizedItem(item_id, True, False, Gender.NOT_SPECIFIED, embedding)
        ...
        assert ret is not None
        assert ret.item_id == item_id
        assert ret.sex == Gender.NOT_SPECIFIED
        assert ret.in_stock == True
        assert ret.embedding_ready == False

        assert len(ret.embedding.data) == len(embedding.data)
        assert all([x == pytest.approx(y)
                    for x, y in zip(ret.embedding.data, embedding.data)])
